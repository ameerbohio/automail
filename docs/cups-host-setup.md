...
## Before you start: the rest of the first deploy

This document covers **only** turning on real paper output — the last step of a
bring-up. Everything that has to be true before it (secrets and the edge TLS
certificate, DNS for the routed hostnames, mailbox/slot provisioning, bringing the
stack up and verifying it) is ordered in
[deploy-checklist.md](deploy-checklist.md). Start there on a fresh host.

In particular, the edge TLS cert (`./infra/certs/gen-edge-certs.sh`) is a
**required** first-deploy step for every host, printing or not — without it
Traefik's `sniStrict` hard-rejects every handshake and the browser never reaches
the portal. That note used to live here as an interim home; it is now
[deploy-checklist.md §1](deploy-checklist.md).
