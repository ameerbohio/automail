...
| **Duplication control** | **B−** | The weak axis, and the reason this track exists. 22 Redis-key string builds, 4 token-hash implementations, 11 near-identical proxy routes, 3 test-server constructors, 5 copies of a shell teardown trap |
...
**The problem.** The same four Redis names are built by string concatenation in
**8 non-test sites across 4 packages**, and **14 more times in tests**:
